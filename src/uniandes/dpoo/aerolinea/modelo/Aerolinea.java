...
public class Aerolinea
{
    ...
    {
    	ArrayList tiquetes = new ArrayList();
    	for (Vuelo vuelo: getVuelos()) {
    		tiquetes.add(vuelo.getTiquetes());
    	}
        return tiquetes;
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	if(getVuelo(codigoRuta, fecha) != null) {
    		throw new Exception("Hay otro vuelo que ya tiene este avion en el mismo intervalo de tiempo.");
    	}
    	else {
    		for (Avion avion: aviones) {
    			if(avion.getNombre() == nombreAvion) {
    				vuelos.add(Vuelo(rutas.get(codigoRuta), fecha, avion));
    			}
    		}
    	}    	
    }
